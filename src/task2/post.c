...
void __compute_post_ratio(linked_list_t *list, uint32_t *current_max, uint32_t *current_max_id);
...
post_t *posts_add_repost(linked_list_t *posts, uint32_t post_id, uint32_t repost_id, uint32_t user_id)
{
...
void post_print_reposts(linked_list_t *posts, uint32_t post_id, uint32_t repost_id)
{
	post_t *post = post_get(posts, post_id, repost_id);
...
uint64_t __compute_common_repost(post_t *post, uint32_t repost_id_1, uint32_t repost_id_2)
{
	...
	while (current != NULL) {
		...
		result = result | __compute_common_repost(current_post, repost_id_1, repost_id_2);
...
uint32_t post_get_common_repost(linked_list_t *list, uint32_t post_id, uint32_t repost_id_1, uint32_t repost_id_2)
{
...
void post_toggle_like(linked_list_t *posts, uint16_t user_id, uint32_t post_id, uint32_t repost_id)
{
	post_t *post = post_get(posts, post_id, repost_id);
...
uint32_t post_get_ratio(linked_list_t *posts, uint32_t post_id)
{
	...
	__compute_post_ratio(post->reposts, &reposts_likes_count, &most_liked_repost_id);
