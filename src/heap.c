...
/**
 * Get a block of a certain size from the heap. The block returned can be larger
...
s64 heap_malloc(heap_t *heap, s64 size)
{
	...
	if (current_block) {
		...
		heap_block_t *new_block = new_heap_block(current_block->start_address,
												 size);
		...
		heap_block_t *remaining_block =
				new_heap_block(current_block->start_address + size,
...
bool heap_free(heap_t *heap, s64 start_address)
{
	...
	if (!byte || byte->holding_block_start_address != start_address)
		return false;

	while (byte && byte->holding_block_start_address == start_address) {
		free(byte);
